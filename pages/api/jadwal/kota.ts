import { searchProvince } from "../../../lib/province"
import { searchCity } from "../../../lib/network"
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const provinceData = searchProvince(searchName.toLowerCase())

  if (provinceData == undefined) {
    return res.status(400).json({ message: "Unknown `provinsi` data, try another key" })
  }

  const listCity = await searchCity(provinceData.id)
